...
{
    if (len == 5 && memcmp(start, "allow", 5) == 0)
        return ALLOW;
    if (len == 5 && memcmp(start, "block", 5) == 0)
        return BLOCK;
    return BLOCK;
}
...
{
    if (len == 4 && memcmp(start, "drop", 4) == 0)
        return STRATEGY_DROP;
    if (len == 3 && memcmp(start, "edt", 3) == 0)
        return STRATEGY_EDT;
    if (len == 3 && memcmp(start, "ecn", 3) == 0)
        return STRATEGY_ECN;
    return -1;
}
